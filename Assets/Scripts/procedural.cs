...
public class procedural : MonoBehaviour {
    ...
    // private int 
    void placeIntersectionAt(int x,int y) {
        for (int xp = 0; xp <= 5; xp++) {
            for (int yp = 0; yp >= -5; yp--) {
                tilemap.SetTile(new Vector3Int(xp+x, yp+y, 0),tileset.GetTile(new Vector3Int(xp-5, yp+5, 0)));
            }
        }
    }
            ...
            if (lb_bound<=ybound) {
                while (true) {
                    ...
                    if (sentry==0) {
                        ...
                        if (l_road_bound!=ybound || m_road_bound!=ybound) continue;
                        l_road_bound+=6;
                        m_road_bound+=6;
                        lb_bound+=6;
            ...
            if (rb_bound<=ybound) {
                while (true) {
                    ...
                    if (sentry==0) {
                        ...
                        if (r_road_bound!=ybound || m_road_bound!=ybound) continue;
                        m_road_bound+=6;
                        r_road_bound+=6;
                        rb_bound+=6;
